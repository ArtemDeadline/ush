...
void mx_check_sub_comm(char **data, t_ush *ush) {
    ...
    // echo '`echo '$(echo "Hello world)"  )' ` '
    // echo "Hello my dear world" $(echo `pwd`  `ls -a` "substituion commands" )
    ...
    for (int i = 0; (*data)[i] != '\0'; i++) {
        ...
        //printf("\nCheck |%c|, dollar_start: %d, grave_start: %d, dollar_sub: %d, qoute: %d, qoute_number: %d\n", (*data)[i], dollar_start, grave_start, dollar_sub, qoute, qoute_number);
        ...
        if(!dollar_start && mx_check_grave_sub_comm(data, com_sub, ush, i, &grave_start, &dollar_sub, &qoute, &qoute_number, &qoute_array) == 1) {
            exit = 1;
            break;
        }
        if(!grave_start && mx_check_dollar_sub_comm(data, com_sub, ush, i, &dollar_start, &dollar_sub, &qoute, &qoute_number, &qoute_array) == 1) {
            exit = 1;
