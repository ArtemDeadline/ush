...
// запустить утилиту только с указанием переменными среды определяется параметрами (name = falue)
// среда унаследуется env полностью игнорируется
...
//выполнение флагов для команды env
